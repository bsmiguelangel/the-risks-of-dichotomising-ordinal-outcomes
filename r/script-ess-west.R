...
# Option 1 (D2): 0 = First two categories; 1 = Last three categories.
y[y > 5] <- NA
y[y == 1 | y == 2] <- 0
y[y == 3 | y == 4 | y == 5] <- 1
...
# Option 2 (D3): 0 = First three categories; 1 = Last two categories.
y[y > 5] <- NA
y[y == 1 | y == 2 | y == 3] <- 0
y[y == 4 | y == 5] <- 1
...
p_fixed <- ggplot(df_plot, aes(x = param, y = mean, color = model)) +
  ...
  scale_color_manual(values = c("Ordinal" = "tomato", 
                     ...
                     labels = c("Ordinal", "Bernoulli (D₂)", "Bernoulli (D₃)")) +
  theme_minimal() +
...
p_probmean_ordinal <- p_probmean_ordinal + labs(tag = "P(θᵣ > 0)")
...
cartography$percentage_CV_D2 <- 100 * cartography$percentage_sd_D2 / cartography$percentage_mean_D2
...
p_percentage_mean_D2 <- p_percentage_mean_D2 + labs(tag = "Bernoulli (D₂)")
...
cartography$percentage_CV_D3 <- 100 * cartography$percentage_sd_D3 / cartography$percentage_mean_D3
...
p_percentage_mean_D3 <- p_percentage_mean_D3 + labs(tag = "Bernoulli (D₃)")
...
cartography$percentage_CV_ordinal_D2 <- 100 * cartography$percentage_sd_ordinal_D2 / cartography$percentage_mean_ordinal_D2
cartography$percentage_CV_bernoulli2 <- 100 * cartography$percentage_sd_bernoulli2 / cartography$percentage_mean_bernoulli2
cartography$percentage_CV_ordinal_D3 <- 100 * cartography$percentage_sd_ordinal_D3 / cartography$percentage_mean_ordinal_D3
...
cartography$border_above_bernoulli1[cartography$low_bernoulli1 > we_mean_post_bernoulli1] <- "#67000D"
cartography$border_above_bernoulli1[cartography$up_bernoulli1 < we_mean_post_bernoulli1] <- "#08306B"

cartography$border_above_ordinal_D2 <- NA
cartography$border_above_ordinal_D2[cartography$low_ordinal_D2 > we_mean_post_ordinal_D2] <- "#67000D"
cartography$border_above_ordinal_D2[cartography$up_ordinal_D2 < we_mean_post_ordinal_D2] <- "#08306B"

cartography$border_above_bernoulli2 <- NA
cartography$border_above_bernoulli2[cartography$low_bernoulli2 > we_mean_post_bernoulli2] <- "#67000D"
cartography$border_above_bernoulli2[cartography$up_bernoulli2 < we_mean_post_bernoulli2] <- "#08306B"

cartography$border_above_ordinal_D3 <- NA
cartography$border_above_ordinal_D3[cartography$low_ordinal_D3 > we_mean_post_ordinal_D3] <- "#67000D"
cartography$border_above_ordinal_D3[cartography$up_ordinal_D3 < we_mean_post_ordinal_D3] <- "#08306B"
...
cartography$difflow_D2 <- apply(diff_D2_post * 100, 2, quantile, probs = 0.025)
cartography$diffup_D2  <- apply(diff_D2_post * 100, 2, quantile, probs = 0.975)

cartography$difflow_D3 <- apply(diff_D3_post * 100, 2, quantile, probs = 0.025)
...
cartography$border_mean_D2[cartography$difflow_D2 > 0] <- "#67000D"
cartography$border_mean_D2[cartography$diffup_D2 < 0] <- "#08306B"

cartography$border_mean_D3 <- NA
cartography$border_mean_D3[cartography$difflow_D3 > 0] <- "#67000D"
cartography$border_mean_D3[cartography$diffup_D3 < 0] <- "#08306B"

cartography$border_sd_D2 <- NA
cartography$border_sd_D2[cartography$difflow_D2 > 0 | cartography$diffup_D2 < 0] <- "#08306B"

cartography$border_sd_D3 <- NA
cartography$border_sd_D3[cartography$difflow_D3 > 0 | cartography$diffup_D3 < 0] <- "#08306B"
...
# Row labels
p_diffmean_D2 <- p_diffmean_D2 + labs(tag = "Ordinal (D₂) - Bernoulli (D₂)")
p_diffmean_D3 <- p_diffmean_D3 + labs(tag = "Ordinal (D₃) - Bernoulli (D₃)")
